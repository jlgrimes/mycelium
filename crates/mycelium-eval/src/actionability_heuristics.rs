...
impl CalibratedActionabilityScorer {
    ...
    pub fn analyze(response: &ProblemResponse) -> ActionabilityAnalysis {
        ...
        let weighted_total = components.structure_clarity.score
            * components.structure_clarity.weight
    ...
    fn score_structure_clarity(response: &ProblemResponse) -> ComponentScore {
        ...
        if response.cross_domain_matches.len() >= 3 {
            ...
            let unique_domains: std::collections::HashSet<String> = response
    ...
    fn score_step_specificity(response: &ProblemResponse) -> ComponentScore {
        ...
        let imperative_patterns = [
            "do", "use", "try", "start", "begin", "focus", "practice", "set",
        ];
        let imperative_count = imperative_patterns
            .iter()
            .filter(|&&word| combined.contains(word))
            .count();
        ...
        let has_sequencing = combined.contains("1.")
            || combined.contains("2.")
    ...
    fn score_contextual_grounding(response: &ProblemResponse) -> ComponentScore {
        ...
        if !mapping.is_empty() {
            ...
            if mapping.contains("->")
                || mapping.contains("maps to")
                || mapping.contains("corresponds")
                || mapping.contains("map ")
                || mapping.contains(" to ")
            {
            ...
            if mapping.contains("confidence")
                || mapping.contains("high")
                || mapping.contains("medium")
            {
                score += 0.2;
        ...
        let context_count = context_indicators
            .iter()
            .filter(|&&word| synthesis.contains(word))
            .count();
    ...
    fn score_temporal_ordering(response: &ProblemResponse) -> ComponentScore {
        ...
        let temporal_count = temporal_words
            .iter()
            .filter(|&&word| synthesis.contains(word))
            .count();
    ...
    fn score_outcome_measurability(response: &ProblemResponse) -> ComponentScore {
        ...
        let measurement_count = measurement_terms
            .iter()
            .filter(|&&word| combined.contains(word))
            .count();
        ...
        let quantity_count = quantity_words
            .iter()
            .filter(|&&word| combined.contains(word))
            .count();
    ...
    fn score_resource_identification(response: &ProblemResponse) -> ComponentScore {
        ...
        let resource_count = resource_terms
            .iter()
            .filter(|&&word| combined.contains(word))
            .count();
        ...
        let capability_count = capability_terms
            .iter()
            .filter(|&&word| combined.contains(word))
            .count();
        ...
        let effort_terms = [
            "time", "effort", "duration", "hours", "days", "weeks", "practice", "training",
        ];
        let effort_count = effort_terms
            .iter()
            .filter(|&&word| combined.contains(word))
            .count();
    ...
    fn generate_improvement_suggestions(components: &ActionabilityComponents) -> Vec<String> {
        ...
        if components.step_specificity.score < 0.6 {
            suggestions.push(
                "Add more specific action steps with imperative language and sequential ordering"
                    .to_string(),
            );
        }
        ...
        if components.temporal_ordering.score < 0.5 {
            suggestions.push(
                "Add temporal sequencing with clear ordering and process flow indicators"
                    .to_string(),
            );
        }
        ...
        if components.outcome_measurability.score < 0.6 {
            suggestions.push(
                "Include measurable outcomes with quantifiable goals and success metrics"
                    .to_string(),
            );
        }
        ...
        if components.resource_identification.score < 0.5 {
            suggestions.push(
                "Identify required resources, tools, skills, and time/effort estimates".to_string(),
            );
        }

        if suggestions.is_empty() {
            suggestions.push(
                "Actionability analysis shows good coverage across all components".to_string(),
            );
        }
